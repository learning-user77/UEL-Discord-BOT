...
async def leave_other_servers(interaction: discord.Interaction):
    ...
    # (To get it: Settings > Advanced > Developer Mode ON > Right click your profile > Copy User ID)
    OWNER_ID = 925817680848617486 
    
    # Check if the person running the command is actually you
    if interaction.user.id != OWNER_ID:
        await interaction.response.send_message("❌ **Access Denied:** You are not the bot owner.", ephemeral=True)
        return
    
    # Acknowledge the command so Discord doesn't timeout
    await interaction.response.send_message("🚨 **Initiating Server Cleansing...** Please wait.", ephemeral=True)
    
    left_count = 0
    error_count = 0
    
    # Loop through every single server the bot is currently in
    for guild in client.guilds:
        # DO NOT leave the server where the command was typed!
        if guild.id == interaction.guild_id:
            continue
            
        try:
            await guild.leave() # The magic line that makes the bot leave
            left_count += 1
